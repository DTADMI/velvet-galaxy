...
export async function isPgFlagEnabled(flagName: string): Promise<boolean> {
  try {
    ...
    const { data } = await supabase.from("feature_flags").select("is_enabled").eq("name", flagName).maybeSingle();
...
export async function setPgFeatureFlag(name: string, enabled: boolean, value?: unknown): Promise<void> {
  const { createClient } = await import("@/lib/supabase/server");
  const supabase = await createClient();
  await supabase.from("feature_flags").upsert({ name, is_enabled: enabled, updated_at: new Date().toISOString() }, { onConflict: "name" });
